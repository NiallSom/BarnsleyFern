import pygame
import random end = False
while not end:
    colour = input("What color? Green(g) or Multi-coloured(m): ")
    if colour =="g" or colour =="m":
        end = True
    else:
        print("Sorry! That option is not available, please try again.") pygame.init()
width,height = 1000,1000 screen = pygame.display.set_mode((width,height))#creates a window for the visual
clock = pygame.time.Clock()#for FPS, quicker if it is not used
pygame.display.set_caption("Chaos fractal")
x,y = 0,0
def main(x,y):
    """
    This is the main function for calculating the x and y values
    """
...
    return ranChoice[0][0],ranChoice[0][1] while 1:
    #game loop
